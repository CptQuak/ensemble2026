...
- **Temporal Aggregation**: Historical data is truncated to the start of the hour. We aggregate target variable `x2` to form `x2_mean`. Noisy variable (`period`) is dropped.
...
- **Time Series Features**:
  ...
  - **Temporal Features**: Includes hour of day, day of week, day of year, month, month boundary indicators, weekend indicators, and Poland-specific public holidays.
  - **Fourier Features**: Cyclical time representations (Sine/Cosine terms for hour, day of week, and day of year) to handle temporal continuity.
...
- **Future Exogenous Variables**: Future static variables (`deviceType`, `region`) remain constant. Future dynamic features are projected into the future using historical averages per device and hour of the day, with fallbacks to overall feature means.
